...
export async function PATCH(
  ...
  try {
    ...
    const payload = normalizeDiretorPayload(body);
    ...
    if (error || !data) {
      return NextResponse.json({ error: "Erro ao atualizar diretor" }, { status: 500 });
    }

    await upsertCurrentMandato(db, params.id, payload);
...
export async function DELETE(
  ...
  if (error) {
    return NextResponse.json({ error: "Erro ao excluir diretor" }, { status: 500 });
  }
